...
int linkedList::checkHead(int num){
    if(num ==1 && head->key%2!=0){return 0;}
    else if (num ==1 && head->key%2==0){return 1;}
    if(num ==2 && head->key%2==0){return 0;}
    else{return 1;}
}
...
void *consumerOdd(void *arg){                                       //our consumer thread for odd, consumer even is close to the same
    while(true) {
        ...
        if (counter <= 0) {
            ...
            pthread_mutex_unlock(&lock);
            sleep(1);}
        else if (list.checkHead(1)==1){                             //checks the head node and if it isnt odd then unlocks and sleeps
            pthread_mutex_unlock(&lock);
            sleep(1);}
